...
pub enum FoundationError {
    ...
    InvalidSequence { top_card: Card, new_card: Card },

    /// Attempted to place card on a completed pile (King is already placed).
    PileComplete { pile_index: u8, new_card: Card },
...
impl fmt::Display for FoundationError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            FoundationError::InvalidPile(index) => {
                write!(f, "Invalid foundation pile index: {}", index)
            }
            ...
            FoundationError::NoAvailablePile { card } => {
                write!(f, "No available foundation pile for {}", card)
            }
        }
    }
}
...
impl Foundations {
    ...
    pub fn remove_card(
        &mut self,
        location: FoundationLocation,
    ) -> Result<Option<Card>, FoundationError> {
        let idx = location.index() as usize;
        let height = self.heights[idx];
    ...
    pub fn is_complete(&self) -> bool {
        self.heights
            .iter()
            .all(|&height| height == FOUNDATION_CAPACITY)
    }
    ...
    pub fn validate_card_placement(
        ...
        if let Some(top_card) = self.get_card(location)? {
            ...
            let expected_rank = Rank::try_from((top_card.rank() as u8) + 1).map_err(|_| {
                FoundationError::InvalidSequence {
                    top_card: *top_card,
                    new_card: *card,
                }
            })?;
...
mod tests {
    ...
    fn foundations_initialize_with_four_empty_piles() {
        let foundations = Foundations::new();
        assert_eq!(FOUNDATION_COUNT, 4, "Foundations should have 4 piles");
        for i in 0..FOUNDATION_COUNT {
            let location = FoundationLocation::new(i as u8).unwrap();
            assert_eq!(
    ...
    fn can_find_pile_for_specific_suit() {
        ...
        foundations
            .place_card_at(location0, Card::new(Rank::Ace, Suit::Hearts))
            .unwrap();

        // Place Ace of Diamonds in second pile
        let location1 = FoundationLocation::new(1).unwrap();
        foundations
            .place_card_at(location1, Card::new(Rank::Ace, Suit::Diamonds))
            .unwrap();

        // Place Ace of Diamonds in second pile
        let location2 = FoundationLocation::new(2).unwrap();
        foundations
            .place_card_at(location2, Card::new(Rank::Ace, Suit::Spades))
            .unwrap();

        // Place Ace of Diamonds in second pile
        let location3 = FoundationLocation::new(3).unwrap();
        foundations
            .place_card_at(location3, Card::new(Rank::Ace, Suit::Clubs))
            .unwrap();
        ...
        let location2 = FoundationLocation::new(2).unwrap();
        foundations
            .place_card_at(location2, Card::new(Rank::Ace, Suit::Clubs))
            .unwrap();
        let location3 = FoundationLocation::new(3).unwrap();
        foundations
            .place_card_at(location3, Card::new(Rank::Ace, Suit::Spades))
            .unwrap();
    ...
    fn find_pile_for_suit_returns_correct_pile_for_existing_suit() {
        ...
        foundations
            .place_card_at(hearts_pile, Card::new(Rank::Ace, Suit::Hearts))
            .unwrap();

        let diamonds_pile = FoundationLocation::new(1).unwrap();
        foundations
            .place_card_at(diamonds_pile, Card::new(Rank::Ace, Suit::Diamonds))
            .unwrap();

        let clubs_pile = FoundationLocation::new(2).unwrap();
        foundations
            .place_card_at(clubs_pile, Card::new(Rank::Ace, Suit::Clubs))
            .unwrap();

        let spades_pile = FoundationLocation::new(3).unwrap();
        foundations
            .place_card_at(spades_pile, Card::new(Rank::Ace, Suit::Spades))
            .unwrap();
        ...
        foundations
            .place_card_at(hearts_pile, Card::new(Rank::Two, Suit::Hearts))
            .unwrap();
        foundations
            .place_card_at(diamonds_pile, Card::new(Rank::Two, Suit::Diamonds))
            .unwrap();
    ...
    fn checking_non_empty_piles_works() {
        ...
        assert_eq!(
            non_empty_locations.len(),
            2,
            "Should have 2 non-empty piles"
        );
        ...
        assert_eq!(
            non_empty_locations[1].1, 1,
            "Second pile should have 1 card"
        );
    }
    ...
    fn auto_place_cards_works() {
        ...
        assert_eq!(
            foundations.get_card(hearts_pile).unwrap().unwrap().rank(),
            Rank::Two
        );
        assert_eq!(
            foundations.get_card(hearts_pile).unwrap().unwrap().suit(),
            Suit::Hearts
        );

        assert_eq!(
            foundations.get_card(spades_pile).unwrap().unwrap().rank(),
            Rank::Ace
        );
        assert_eq!(
            foundations.get_card(spades_pile).unwrap().unwrap().suit(),
            Suit::Spades
        );
        ...
        assert!(matches!(
            result,
            Err(FoundationError::InvalidSequence { .. })
        ));
    }
}
